...
There is a test suite which is implemented with googletests. The code is 100\% covered by the tests. Documentation is created from appropriately formatted comments by Doxygen.\hypertarget{index_autotoc_md1}{}\doxysection{How to use this site}\label{index_autotoc_md1}
...
\DoxyCodeLine{and its reverse complement: ctg}
\DoxyCodeLine{inserting pattern: ag}
\DoxyCodeLine{and its reverse complement: ct}
\DoxyCodeLine{inserting pattern: gca}
\DoxyCodeLine{and its reverse complement: tgc}
\DoxyCodeLine{inserting pattern: ca}
\DoxyCodeLine{and its reverse complement: tg}
